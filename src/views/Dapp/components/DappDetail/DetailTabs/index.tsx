...
const DetailTabs = (props: Props) => {
  ...
  const { logo, category, overviewTitle, chain_id, default_chain_id, dapp_network = [] } = props;

  const { loading, historyList, pageTotal, pageIndex, fetchHistoryList, total } = useMyHistory({
    category,
    id: props.id,
    chainId: chain_id
  });
    ...
    <StyledContainer>
      <StyledTabContainer>
        <StyledTabs className="history-tabs">
          {TABS.map((item) => (
            <StyledTab className="history-tab" key={item.key} data-bp={item?.bp} onClick={() => setCurrTab(item.key)}>
        ...
        {account && isShowPortfolio && (
          <StyledPortfolioButton onClick={onPortfolio}>
            Portfolio
            <ArrowLineIcon />
          </StyledPortfolioButton>
        )}
        ...
        <AnimatePresence mode="wait">
          {currTab === TABS[0].key && (
            <Animate key="overview">
              <Overview title={overviewTitle} {...props} chain_id={chain_id || default_chain_id} />
            </Animate>
          )}
          {currTab === TABS[1].key && (
