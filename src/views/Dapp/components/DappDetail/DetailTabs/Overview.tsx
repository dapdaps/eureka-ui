      ...
      if (json) {
        ...
        if (!json.address && chain_id) {
          ...
          if (currChainTokenList) {
            const currToken = Object.values(currChainTokenList).find(
              (t) => t.symbol.toUpperCase() === json.symbol.toUpperCase()
            );
                  ...
                  <StyledTokenInfo>
                    ...
                    <StyledTokenAddress>
                      {nativeCurrency?.address
                        ? nativeCurrency?.address.substring(0, 5) +
                          '...' +
                          nativeCurrency?.address.substring(nativeCurrency?.address.length - 6)
                        : '-'}
                    </StyledTokenAddress>
                    {nativeCurrency?.address && (
                      <>
            ...
            {airdropData.quests && airdropData.quests.length > 0 && (
              <StyledAirdropBody>
                <StyledAirdropActions>
                  ...
                  {!airdropLoading && (
                    <StyledAirdropActionsText>
                      <StyledAirdropActionsTextPrimary>{airdropData.completedCount}</StyledAirdropActionsTextPrimary>
                      <span>/</span>
                ...
                <StyledAirdropActionsSub>
                  ...
                  &nbsp; The following actions are for reference only, and there is no guarantee that official airdrops
                  can be obtained
                </StyledAirdropActionsSub>
                  ...
                  >
                    <div>
                      <StyledAirdropShadow className={item.completed ? 'finished' : ''}>
                        ...
                        {item.completed && <StyledAirdropIcon url="/images/alldapps/icon-checked.svg" />}
                      </StyledAirdropShadow>
                      <StyledAirdropTitle>{item.name}</StyledAirdropTitle>
                    </div>
                    <StyledFlex justifyContent="end" gap="20px">
                      {!item.completed && (
                        <StyledAirdropReload title="refresh" onClick={handleReload}>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="18"
                            height="18"
      ...
      <StyledSummaryAddIcon $direction={Big(value).gte(0) ? 'up' : 'down'}>
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="8" viewBox="0 0 10 8" fill="none">
          <path
            ...
            fill="currentColor"
            stroke="url(#paint0_linear_16163_4093)"
          />
          <defs>
            <linearGradient
