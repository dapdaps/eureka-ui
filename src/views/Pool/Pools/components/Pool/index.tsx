...
const Pool = ({ token0, token1, chainId, fee, tickLower, tickUpper, liquidity, onClick }: any) => {
  const _token0 = useToken(token0, chainId);
  const _token1 = useToken(token1, chainId);
  const { info, loading } = usePoolInfo({ token0: _token0, token1: _token1, fee });
    ...
    <StyledContainer onClick={onClick}>
      ...
      <StyledRange>
        ...
        <span className="range-item">
          ...
          {isFullRange
            ? 0
            : formateValue(1 / tickToPrice({ tick: tickLower, token0: _token0, token1: _token1 }), 3)}{' '}
