...
export function tickToPrice({ tick, decimals0, decimals1, isReverse, isNumber }: any) {
  ...
  const price1 = price0.eq(0) ? new Big(2).pow(96) : new Big(1).div( price0);
  if (isReverse) {
    if (price0.lt(0.001) && !isNumber) return '<0.001';
    return !isNumber ? price0.toFixed(3) : price0.toFixed(32);
  }
  if (price1.lt(0.001) && !isNumber) return '<0.001';
  return !isNumber ? price1.toFixed(3) : price1.toFixed(32);
}
