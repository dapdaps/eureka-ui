  ...
  const queryRequests = useCallback(
    async () => {
      ...
      try {
        ...
        const response = await fetch('https://ded76165a2fb6f7887260a3a0f626de7.thegraph.chainnodes.org/subgraphs/name/etherfi/etherfi-subgraph-v0-8-2', {
          method: 'POST',
          headers: {
        ...
        Promise.all(promiseArray).then(getClaimableAmountResults => {
          setRequests(_requests.map((request: any, index: number) => {
