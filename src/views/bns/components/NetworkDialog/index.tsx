...
const iconPlus = (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path
      d="M10.464 14.876V11.69H7.35V10.286H10.464V7.1H12.012V10.286H15.126V11.69H12.012V14.876H10.464Z"
      fill="#979ABE"
    />
  </svg>
);
...
const NetworkDialog = ({
  ...
  const filterCoinTypeList = useMemo(
    () => COIN_TYLE_LIST.filter((coinType) => typeof addresses[coinType] === 'object'),
    [bnsName, addresses],
  );
  ...
  const addValues = useMemo(() => {
    ...
    Object.keys(addresses).forEach((coinType) => {
      if (
        typeof addAddresses[coinType] === 'object' &&
        addresses[coinType] &&
        addresses[coinType][0] &&
        !addresses[coinType][1]
      ) {
  ...
  const handleConfirm = async function () {
    ...
    Object.keys(addValues).forEach((coinType) => {
      ...
      args.push(iface.encodeFunctionData('setAddr', [nameHash, coinType, value]));
    });
    ...
    <StyledDialog>
      <StyledDialogContainer>
        <StyledDialogHead>
          {saveStatus === 1 ? (
            <StyledFlex $gap="15px">
              <StyledSvg style={{ cursor: 'pointer' }} onClick={() => setSaveStatus(0)}>
                {iconBack}
              </StyledSvg>
            ...
            <StyledText $size="20px" $weight="700">
              Save Chages
            </StyledText>
          ) : (
            <StyledWrapper></StyledWrapper>
          )}
          <StyledSvg style={{ cursor: 'pointer' }} onClick={() => onClose && onClose()}>
            {iconClose}
          </StyledSvg>
        </StyledDialogHead>
        {saveStatus === 0 && (
          <StyledDialogBody style={{ width: 536 }}>
            <StyledFlex $direction="column" $gap="10px">
              <Image src={bnsAvatar} width={62} alt="bnsAvatar" />
              <StyledText $size="18px" $weight="700">
                {bnsName.name}
              </StyledText>
              <StyledButton
                $width="102px"
              ...
              >
                <StyledText $color="#979ABE" $size="12px">
                  Set as Primary
                </StyledText>
              </StyledButton>
            </StyledFlex>
            <StyledPlusButton onClick={() => setSaveStatus(1)} style={{ marginTop: 23, marginBottom: 30 }}>
              <StyledSvg className="gray">{iconPlus}</StyledSvg>
              <StyledSvg className="white">{iconWhitePlus}</StyledSvg>
            </StyledPlusButton>
                  ...
                  return (
                    <StyledFlex key={coinType} $direction="column" $gap="10px" style={{ width: '100%' }}>
                      <StyledFlex $justify="flex-start" $gap="6px" style={{ width: '100%' }}>
                        ...
                        <StyledText $size="14px" $weight="500" $line="120%">
                          {coin.label}
                        </StyledText>
                      </StyledFlex>
                      <StyledAddressInputWrapper>
                        <StyledAddressInput
                          value={addresses[coinType][0]}
                          onChange={(event) => handleInputChange(event, coinType)}
                        />
                        <StyledSvg style={{ cursor: 'pointer' }} onClick={() => handleDelete(coinType)}>
                          {iconDelete}
                        </StyledSvg>
                      </StyledAddressInputWrapper>
                      {addresses[coinType][1] && (
                        <StyledFlex
                          $justify="flex-start"
                          $gap="6px"
                          style={{ width: '100%', color: 'rgb(247, 187, 67)' }}
                        >
                          <StyledSvg>{iconWarning}</StyledSvg>
                          <StyledText $color="rgb(247, 187, 67)" $size="12px" $weight="700">
                            Invalid address
                          </StyledText>
                        </StyledFlex>
                      )}
        ...
        {saveStatus === 1 && (
          <StyledDialogBody style={{ width: 536, paddingTop: 20 }}>
            ...
            {Object.keys(addAddresses).length > 0 ? (
              <StyledButton onClick={handleAdd} $background="#2E3142" style={{ marginTop: 20 }}>
                <StyledText $size="16px" $weight="700">
                  Add ({Object.keys(addAddresses).length})
                </StyledText>
              </StyledButton>
            ) : (
              ...
              <StyledFlex $justify="space-between" $align="flex-start" style={{ width: '100%' }}>
                ...
                <StyledFlex $direction="column" $align="flex-end" $gap="16px">
                  {Object.keys(addValues).map((coinType) => {
                    const coin = COIN_TYLE_MAP[coinType];
                    return (
                      <StyledText $size="16px" key={coinType}>
                        {coin.label}: {_.truncate(addValues[coinType], { length: 10 })}
                      </StyledText>
                    );
                  })}
                </StyledFlex>
