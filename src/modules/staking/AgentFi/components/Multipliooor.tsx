...
const DEPOSIT_POOL_ABI = [
  {
    inputs: [{ internalType: 'address', name: 'rootAgentAddress', type: 'address' }],
...
export default memo(function Multipliooor(props: any) {
  ...
  const handleSubmit = () => {
    ...
    const getTx = (gas) => {
      ...
      contract
        .createMultipliooorAgentForRoot(...params, contractOption)
        .then((tx) => {
          tx.wait()
            .then((res) => {
    ...
    const estimateGas = () => {
      contract.estimateGas
        .createMultipliooorAgentForRoot(...params, { value: parseUnits(amount, eth?.decimals || 18) })
        .then((gas) => {
          getTx(gas);
        })
        .catch((err) => {
  ...
  useEffect(() => {
    const eth = StakeTokens.find((it) => it.symbol === 'ETH');
    eth &&
      getTokenBalance(eth).then((value) => {
        updateState({
          balance: value
        });
      });
  ...
  return (
    <StyledContainer>
      <StyledDexTips>You can withdraw any remaining funds at the end of the Blast Multiplier program.</StyledDexTips>
      <StyledButton disabled={pending || !balance || !rootAgent.agentAddress} onClick={handleSubmit}>
        {pending ? (
          <Loading size={16} />
        ) : Big(balance).gte(Big(amount)) ? (
          'DEPOSIT ' + amount + ' ETH'
        ) : (
