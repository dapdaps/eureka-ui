...
export default memo(function Detail(props: any) {
  ...
  const handleApprove = (isToken0) => {
    ...
    const contract = new ethers.Contract(addresses[_token], abi, Ethers.provider().getSigner());
  ...
  const handleDeposit = () => {
    ...
    const amount0Max = Big(amount0).mul(Big(10).pow(decimals0)).toFixed(0);
    const amount1Max = Big(amount1).mul(Big(10).pow(decimals1)).toFixed(0);
  ...
  const handleWithApprove = () => {
    ...
    const burnAmount = Big(lpAmount).mul(Big(10).pow(18)).toFixed(0);
    const toastId = toast?.loading({
      title: `Approve ${_token}`
    });
    updateState({
    ...
    const contract = new ethers.Contract(ethers.utils.getAddress(vaultAddress), abi, Ethers.provider().getSigner());
    contract
  ...
  const handleWithdraw = () => {
    ...
    contract.callStatic.removeLiquidity(vaultAddress, burnAmount, 0, 0, sender).then((result) => {
      contract
            ...
            <>
              ...
              <StyledButtonList>
                {isWithApprived && !isWithApproving ? (
                  ...
                  <StyledButton
                    disabled={
                      isWithdrawInsufficient || isWithApprived || isWithApproving || !Big(Number(lpAmount)).gt(0)
                    }
                    onClick={() => handleWithApprove()}
                  >
                    {isWithApproving ? (
                      <Loading />
                    ) : (
