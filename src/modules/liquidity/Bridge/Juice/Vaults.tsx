...
export default memo(function Vaults(props: any) {
  ...
  function renderPointValue(vault, point) {
    if (point.type === "deposited") {
      return point.value ? ("$" + Big(point.value).times(prices[vault.token0]).toNumber().toLocaleString('en-us', {
  ...
  return (
    <StyledContainer>
      <StyledTips>Farm yield and earn points by deploying into different Dapps across the Blast Layer 2 ecosystem.</StyledTips>
      <StyledVaults>
        {
