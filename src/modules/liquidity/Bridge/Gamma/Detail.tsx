...
export default memo(function Detail(props: any) {
  ...
  const { from, data, toast, prices, addresses, defaultDex, proxyAddress, addAction, userPositions, ICON_VAULT_MAP } =
    props;
  ...
  const handleDeposit = () => {
    ...
    handleTokenChange(
      ...
      (amount: string, otherAmount: string) => {
        ...
        const proxyContract = new ethers.Contract(proxyAddress, proxyAbi, provider.getSigner());
        proxyContract
  ...
  const handleGetAmount = (contract: any, params: any, callback: any) => {
    contract.callStatic.withdraw(...params).then(callback);
  };
  const handleWithdraw = () => {
    ...
    const abi = ['function withdraw(uint256, address, address,uint256[4] memory) external returns (uint256, uint256)'];
