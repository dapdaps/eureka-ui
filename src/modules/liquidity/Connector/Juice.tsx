...
export default memo(function JuiceConnector(props) {
  ...
  const { ICON_MAP, SYMBOL_LIST, POOLS_MAPPING, SYMBOL_NAME_MAPPING } = dexConfig;
  const [state, updateState] = useMultiState<any>({
    categoryList: ['Vaults', 'Borrowers', 'Positions'],
    categoryIndex: 0,
    symbolIndex: 0,
    checkedVault: null,
  ...
  const { vaults, PROXY_ADDRESS, LENDING_POOL_ADDRESS, SYMBOL_ADDRESS } = POOLS_MAPPING[symbolIndex];
  ...
  function handleGetSubAccount() {
    ...
    const contract = new ethers.Contract(ethers.utils.getAddress(PROXY_ADDRESS), abi, provider);
    contract
      .getAccount(sender)
      .then((result) => {
        updateState({
