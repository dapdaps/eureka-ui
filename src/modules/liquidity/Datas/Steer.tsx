...
export default function Data(props: any) {
  const { pairs, sender, provider, addresses, allData, onLoad, curChain, FEE_APR_URL, multicallAddress, prices } =
    props;
  const MULTICALL_ABI = [
    {
      inputs: [
  ...
  function handleGetSpecial(i) {
    ...
    multicallv2(
      ...
      (result) => {
        ...
        dataList[i].tvlUSD = Big(ethers.utils.formatUnits(firstBalanceResult[0], data.decimals0))
          .times(prices[data.token0])
          .plus(Big(ethers.utils.formatUnits(secondBalanceResult[0], data.decimals1)).times(prices[data.token1]))
          .toFixed(2);
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      if (data.poolAddress === '0x47d7b9510ae2835c7c293825641a5427226d34cb') {
        ...
        dataList[i].tvlUSD = Big(ethers.utils.formatUnits(token0Balance ?? 0, data.decimals0))
          .times(prices[data.token0])
          .plus(Big(ethers.utils.formatUnits(token1Balance ?? 0, data.decimals1)).times(prices[data.token1]))
          .toFixed(2);
      }
    }
    formatedData('getTvlUSD');
