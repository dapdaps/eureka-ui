  ...
  useEffect(() => {
      ...
      multicall({
        ...
        .then((res: any) => {
          ...
          for (let i = 0, len = res.length; i < len; i++) {
            if (i === res.length - 1) {
              _accountRewards.reward = res[i]?.[0] ? ethers.utils.formatUnits(res[i]?.[0]._hex, 18) : '0';
              count++;
        ...
        .then((res: any) => {
          ...
          if (nativeOToken) {
            provider.getBalance(nativeOToken).then((rawBalance: any) => {
              _liquidity[nativeOToken] = rawBalance ? ethers.utils.formatUnits(rawBalance._hex, 18) : 0;
              count++;
        ...
        .then((res: any) => {
          ...
          if (nativeOToken) {
            provider.getBalance(account).then((rawBalance: any) => {
              _underlyingBalance[nativeOToken] = rawBalance ? ethers.utils.formatUnits(rawBalance._hex, 18) : '0';
              count++;
    ...
    const getCTokenReward = ({ price, cTokens, index }: { price: number; cTokens: any; index: number }) => {
      ...
      multicall({
        ...
        .then((res: any) => {
          const borrow = res[0][0] ? Big(ethers.utils.formatUnits(res[0][0]._hex, 18)).mul(price) : Big(0);
          const supply = res[1][0] ? Big(ethers.utils.formatUnits(res[1][0]._hex, 18)).mul(price) : Big(0);
          _rewards[token] = {
