...
export default function RadiantData(props: any) {
  const {
    oracleAddress,
        ...
        .then((res: any) => {
          const scale =
            ...
            oracleAddress === '0x8429d0AFade80498EAdb9919E41437A14d45A00B'
              ? 1000000000000000000
              : 100000000;
          const parsedRes = res.map((price: number) => {
        ...
        .then((res: any) => {
          ...
          addresses.forEach((address: string, index: number) => {
            ...
            for (const m in markets) {
              if (
                markets[m].underlyingToken.address === 'native' &&
                address === '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'
              ) {
                const parsedValue = Big(value.toString())
                  .div(Big(10).pow(markets[m].underlyingToken.decimals))
                  .toFixed();
                markets[m].userUnderlyingBalance = parsedValue;
                break;
              }
            }
          });
