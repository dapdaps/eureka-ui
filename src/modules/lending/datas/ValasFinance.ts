...
export default function ValasFinanceData(props: any) {
  ...
  useEffect(() => {
    ...
    const getRewardPrice = () => {
      ...
      rewardToken.price = prices[rewardToken.symbol] || 0;
    ...
    const getRewardsData = () => {
      ...
      const addrs = Object.values(markets)
        .map((item: any) => [item.address, item.variableDebtTokenAddress])
        .flat();

      RewardContract.claimableReward(account, addrs)
        .then((res: any) => {
          const total = res.reduce((pre: any, cur: any) => {
            const curFormat = ethers.utils.formatUnits(cur, rewardToken.decimals);

            pre = Big(pre).plus(Big(curFormat)).toFixed();
            return pre;
          }, 0);
