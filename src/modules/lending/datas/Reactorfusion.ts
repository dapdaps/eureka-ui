  ...
  useEffect(() => {
    ...
    const formatedData = (key: string) => {
      ...
      Object.values(_cTokensData).map((market: any) => {
        ...
        totalSupplyUsd = totalSupplyUsd.plus(Big(market.totalSupply).mul(underlyingPrice));
        totalBorrowUsd = totalBorrowUsd.plus(Big(market.totalBorrows).mul(underlyingPrice));
