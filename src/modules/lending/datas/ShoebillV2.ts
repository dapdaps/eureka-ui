...
export default function ShoebillV2Data(props: any) {
  const { multicallAddress, account, update, name, onLoad, multicall, markets, prices, provider } = props;
    ...
    const getCTokenData = (oToken: any) => {
      ...
      multicall({
        ...
        .then((res: any) => {
          ...
          _loanToValue[oToken.address] = ethers.utils.formatUnits(res[7]?.[0]._hex, 16);
