  ...
  useEffect(() => {
    ...
    const formatedData = (key: any) => {
      ...
      Object.values(_cTokensData).forEach((market: any) => {
        ...
        let supplyApy = Big(market.supplyRatePerTimestamp)
          .mul(60 * 60 * 24)
          .plus(1)
          .pow(365)
          .minus(1)
        ...
        if (market.symbol === 'rETH') {
          supplyApy = Big(supplyApy).times(0.2974988965125649);
        }

        let borrowApy = Big(market.borrowRatePerTimestamp)
          .mul(60 * 60 * 24)
          .plus(1)
          .pow(365)
          .minus(1)
          .mul(100);
        if (market.symbol === 'rETH') {
          borrowApy = Big(borrowApy).times(0.29165704474485915);
        }
    ...
    const getCTokenData = (oToken: any) => {
      ...
      multicall({
        ...
        .then((res: any) => {
          const exchangeRateStored = ethers.utils.formatUnits(res[0] ? res[0][0]._hex || 0 : 0, 18);
          ...
          const _rateModelSlopeAddress = res[7][0];
          const rateCalls = [
            {
              address: _rateModelSlopeAddress,
              name: 'getBorrowRate',
              params: [
                res[4] ? res[4][0] || '0' : '0',
                res[2] ? res[2][0] || '0' : '0',
