  ...
  useEffect(() => {
    ...
    const getWalletBalance = () => {
      ...
      multicall({
        ...
        .catch((err: any) => {
          console.log('getWalletBalance err', err);
          setTimeout(() => {
            getWalletBalance();
          }, 500);
        });
    ...
    const getCTokenData = (oToken: any) => {
      ...
      multicall({
        ...
        .then((res: any) => {
          oTokensLength--;
          const exchangeRateStored = res[0] && res[0][0]
            ? ethers.utils.formatUnits(
              res[0][0]._hex,
              10 + oToken.underlyingToken.decimals,
            )
            : '0';
          const totalSupply = res[1] && res[1][0]
          ...
          _cTokensData[oToken.address] = {
            ...
            totalBorrows: res[2] && res[2][0]
              ? ethers.utils.formatUnits(
                res[2][0]._hex,
                oToken.underlyingToken.decimals,
              )
              : '0',
            supplyRatePerBlock: res[6] && res[6][0]
              ? ethers.utils.formatUnits(res[6][0]._hex, 18)
              : '0',
            borrowRatePerBlock: res[5] && res[5][0]
              ? ethers.utils.formatUnits(res[5][0]._hex, 18)
              : '0',
            userSupply: Big(userSupply).mul(exchangeRateStored).toString(),
            userBorrow: res[4] && res[4][0]
              ? ethers.utils.formatUnits(
                res[4][0]._hex,
                oToken.underlyingToken.decimals,
              )
