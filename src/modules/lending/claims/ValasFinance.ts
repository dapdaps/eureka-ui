...
export default function ValasFinanceClaim(props: any) {
  ...
  useEffect(() => {
    ...
    const nativeReward: any = Object.values(markets).find((item: any) => item.symbol === 'valBNB');
    ...
    const CollateralContract = new ethers.Contract(dapp.rewardAddress, CLAIM_ABI, provider.getSigner());
