...
export default function MedalCard({ medal, style, barWidth, threshold, today_days }: PropsType) {
  return (
    <StyledMedalCard style={style}>
      <StyledFlex gap='15px'>
        <StyledMedalImage style={{ filter: today_days < threshold ? 'grayscale(100%)' : 'grayscale(0)' }} src={medal?.logo} />
      ...
      <ProgressBar quantity={+today_days} total={threshold} barWidth={barWidth} />
    </StyledMedalCard>
  )
}