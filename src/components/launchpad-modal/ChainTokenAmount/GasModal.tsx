...
export default function GasModal({ onClick, onClose, fromChain, fromToken, toAddress, toChain, maxBalance }: Props) {
  ...
  return (
    <Modal
      title="Refuel Gas Token"
      onClose={() => {
        !isLoading && onClose();
      }}
    >
      <Tip>
        Transfer {fromToken?.symbol} for {toChain?.nativeCurrency.symbol} to cover gas fee on Base.
      </Tip>
      <RefuelAmount>
        ...
        <div className="transter-detail">
          ...
          <div>
            {balanceFormated(receive)} {toChain?.nativeCurrency.symbol}
          </div>
        </div>
      </RefuelAmount>
